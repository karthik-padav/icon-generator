...
export function processSVG(_svg: any) {
  ...
  if (tempEl.querySelector(`#${WRAPPER_SVG}`)) {
    ...
    foreignObjectDiv?.setAttribute(
      "style",
      "text-align:center;overflow:hidden;position:absolute;top:0;bottom:0;right:0;left:0;margin:auto;",
    );
...
export function downloadPng(filename: string, ext: string) {
  ...
  img.src =
    ...
    btoa(unescape(encodeURIComponent(clonedElement)));
...
export function updateSVGControl({
  ...
  switch (key) {
    ...
    case "borderColor":
      ...
      break;
    case "bgSize":
      fbd?.setAttribute("width", `${value}%`);
      fbd?.setAttribute("height", `${value}%`);
